...
router.post("/categories/:categoryId/menus", async (req, res, next) => {
  try {
    // 유효성 검사를 합니다.
    const { name, description, image, price, status } = await checkOrder.validateAsync(req.body);
    ...
    if (!name || !description || !image || !price || !status) {
      return res
        .status(400)
...
router.patch(
  ...
  async (req, res, next) => {
    try {
      // 유효성 검사를 합니다.
      const { name, description, price, order, status } = await checkOrder.validateAsync(req.body);
      // 메뉴 가격이 0으로 표시되면 오류 메시지를 반환합니다.
      if (price === 0) {
        return res
          .status(400)
